...
(function(){
  ...
  const filterWrap = (function(){
    ...
    const filters = [
      ...
      { key: 'completed', label: '対応完了' },
      { key: 'cancelled', label: '対応不要' },
    ];
    let current = 'all';
    const buttons = [];
    const setActive = (btn, active)=>{
      btn.className = active ? 'px-3 py-1.5 rounded bg-gray-900 text-white' : 'px-3 py-1.5 rounded bg-white text-gray-700 border border-gray-300 hover:bg-gray-50';
    };
    filters.forEach(f => {
      const b = document.createElement('button');
      b.type = 'button';
      b.textContent = f.label;
      setActive(b, f.key === current);
      b.addEventListener('click', () => {
        current = f.key;
        buttons.forEach(x => setActive(x, false));
        setActive(b, true);
        renderList();
      });
      buttons.push(b);
    ...
    return { el: container, getCurrent: ()=>current };
  })();

  root.parentElement.insertBefore(filterWrap.el, root);

  // ステータス順でグルーピング表示（未対応→対応中→完了→不要）
  const order = { 'pending': 0, 'in-progress': 1, 'completed': 2, 'cancelled': 3 };
  function renderList(){
    root.innerHTML = '';
    const currentFilter = filterWrap.getCurrent();
    const list = [...merged]
      .filter(d => currentFilter === 'all' ? true : d.status === currentFilter)
